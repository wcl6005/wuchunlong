...
function del_db {
    ...
    rm "-rf" "${BASE_DIR}/mysite/reptilian/migrations/0*"
    ls "${BASE_DIR}/mysite/reptilian/migrations/"
...
function write_data_db {
    ...
    #python "${BASE_DIR}/mysite/initdb_user.py"
...
function launch_webapp {
    python "${BASE_DIR}/mysite/manage.py" "runserver" "8000"
}
#############
# Main
#############
...
if [ "${OPT_ENV_FORCE}x" == "-cax" ];then    
    creator_db
    python "${BASE_DIR}/mysite/manage.py" "createsuperuser" #创建超级用户。一般情况不用操作，已经在write_data_db中实现此功能了。
fi
