...
jkWebSocket = {
	adress:"ws://comet"+EVENT_HOST_PRE,//上线替换正式
	...
	unread: function() {
		$.ajax({
			type: "get",
			data:{'type':'jsonp'},
			url: "http://www"+EVENT_HOST_PRE+"/message/v1/unread?callback=?",
...
//				console.log("网络错误")
	...
	creatMessageList: function(data) {	
		...
		$('.news-list li a').click(function() {
			...
			$.ajax({
				type: "get",
				url: "http://www"+EVENT_HOST_PRE+"/message/v1/read?callback=?",
				data: {
					"ids": id,
					"type":"jsonp"
				},
...
//							unreadNumber--;
	...
	messageList: function() {
		$.ajax({
			type: "get",
			url: "http://www"+EVENT_HOST_PRE+"/message/v1/unread_list?callback=?",
			success: function(data) {
				if (data.code == 200) {
					var newslist = $('.news-list');
					newslist.empty();
